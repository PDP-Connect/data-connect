...
The story: MIVHED needs a household's SIUBEN classification to assess a housing application. Instead of asking the citizen for a paper certificate, it asks for permission to read exactly those records, once, for that stated purpose. The citizen signs in with Cuenta Única, reviews the request, approves it, and the form fills itself. The citizen can revoke the permission afterwards.
...
3. **Authorization request.** "This is the one new screen. It says who is asking, for what purpose, from which institution, exactly which fields, and that it's a single read." Point out that the INTRANT licence is not requested. Click **Continuar**.
4. **Confirmation.** "She confirms the exact request the server recorded, including when it expires. The technical record is there for auditors, collapsed." Click **Autorizar**.
5. **Back at MIVHED.** The form is filled from SIUBEN, each field tagged as sourced from SIUBEN, with the authorization's identifier, purpose and access type. "Once-only: she didn't carry a certificate, and SIUBEN released only what she approved."
6. **Revocation.** Click **Ver o revocar esta autorización**, then tick the confirmation and **Revoke grant**. "Every release is logged and she can withdraw it."
...
- Step 6: The citizen sees and revokes the grant.

Not shown: the X-Road hop between MIVHED and SIUBEN. Here the resource server answers directly.

## Known limits

- The requester cannot set the end date. A single read expires 24 hours after approval, set by the server.
- The grant page used for revocation is the English operator console.
