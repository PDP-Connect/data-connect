...
const FULL_CHROMIUM_COMMAND_RE =
  /\/(?:opt\/patchright-browsers|root\/\.cache\/ms-playwright)\/[^\s]+\/chrome-(?:linux64|linux)\/chrome(?:\s|$)/u;
...
function assertPatchrightRuntime(): void {
  const packageManifest = "/app/reference-implementation/package.json";
  const require = createRequire(packageManifest);
