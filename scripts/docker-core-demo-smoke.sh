...
openssl req -x509 -newkey rsa:2048 -nodes -keyout "$TEMP_ROOT/tunnel.key" -out "$TEMP_ROOT/tunnel.crt" -days 1 -subj "/CN=$TUNNEL_HOST" -addext "subjectAltName=DNS:$TUNNEL_HOST" >/dev/null 2>&1
...
docker run --detach --name "$PROXY_CONTAINER" \
  --network "$NETWORK" --network-alias "$TUNNEL_HOST" \
