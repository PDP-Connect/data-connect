...
async function mint(asUrl: string, spotifyManifestPath: string): Promise<string> {
  const spotifyManifest = JSON.parse(await (await import("node:fs")).promises.readFile(spotifyManifestPath, "utf8"));
  ...
  if (!connectorId) {
    throw new Error("spotify manifest must resolve to a canonical connector key");
  }
  ...
  if (!code) {
    throw new Error("HTML approve response did not embed a cex_... exchange code");
  }
  return code;
}
...
async function main(): Promise<void> {
  ...
  const introspectionCallerCredentials = JSON.parse(introspectionCallerCredentialsArg);
