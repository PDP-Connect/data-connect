...
const CONSENT_HANDOFF_RESTART_FIXTURE_PATH = fileURLToPath(
	new URL(
		"./fixtures/consent-handoff-restart-server-fixture.mjs",
		import.meta.url,
	),
);
const SPOTIFY_MANIFEST_PATH = join(
	REFERENCE_IMPL_DIR,
	"fixtures/seed-manifests/spotify.json",
);
...
async function runConsentHandoffRestartFixture(
	dbPath: string,
	go:
		| { op: "exchange"; code: string }
		| { op: "mint"; spotifyManifestPath: string },
	...
	function nextLine(): Promise<string> {
		if (lines.length > 0) {
			...
			assert.ok(
				line !== undefined,
				"a line just confirmed present in the buffer must be shiftable",
			);
	...
	assert.ok(
		!("error" in result),
		`fixture reported an error: ${JSON.stringify(result)}`,
	);
	return result;
}
...
async function initiateGrantRequest(
	asUrl: string,
...
async function approveReviewedHtml(
	asUrl: string,
	requestUri: string,
): Promise<Response> {
...
async function approveReviewedJson(
	asUrl: string,
	requestUri: string,
): Promise<Response> {
	const revision = await reviewConsent(asUrl, requestUri);
...
async function withHarness(
	...
	const spotifyManifest = JSON.parse(
		readFileSync(
			join(REFERENCE_IMPL_DIR, "fixtures/seed-manifests/spotify.json"),
			"utf8",
		),
	) as SpotifyManifest;
...
test("security: harden consent token handoff", async (t) => {
	await t.test(
		"HTML approve does not embed the bearer; JSON approve still returns it",
		async () => {
			await withHarness(async ({ asUrl, spotifyManifest }) => {
				...
				const initiateForJson = await initiateGrantRequest(
					asUrl,
					spotifyManifest,
				);
				...
				assert.equal(
					typeof jsonBody.token,
					"string",
					"JSON branch SHALL still return the bearer",
				);
				...
				assert.equal(
					jsonBody.code,
					undefined,
					"JSON branch SHALL NOT include an exchange code",
				);

				// Now drive a fresh approval through the HTML branch.
				const initiateForHtml = await initiateGrantRequest(
					asUrl,
					spotifyManifest,
				);
				...
				const revisionMatch = htmlReview.match(
					APPROVAL_REVIEW_REVISION_PATTERN,
				);
				assert.ok(
					revisionMatch?.[1],
					"review HTML SHALL carry the approval review revision",
				);
				...
				assert.ok(
					...
					"HTML branch SHALL render an HTML document",
				);
	...
	await t.test(
		"manual HTML exchange is single-use without exposing recovery proof",
		async () => {
			await withHarness(async ({ asUrl, spotifyManifest }) => {
				...
				const revisionMatch = reviewHtml.match(
					APPROVAL_REVIEW_REVISION_PATTERN,
				);
				assert.ok(
					revisionMatch?.[1],
	...
	await t.test(
		"response-loss retry with the same out-of-band proof returns the same durable result",
		async () => {
			await withHarness(async ({ asUrl, spotifyManifest }) => {
				const initiate = await initiateGrantRequest(asUrl, spotifyManifest);
	...
	await t.test(
		...
		async () => {
			await withHarness(async ({ asUrl, spotifyManifest }) => {
				...
				assert.equal(
					responses.filter((response) => response.status === 410).length,
					7,
				);
				...
				const stored = getDb()
					.prepare(
						"SELECT COUNT(*) AS n, COUNT(redeemed_at) AS redeemed FROM consent_exchange_codes",
					)
	...
	await t.test(
		"reissuing a handoff invalidates older outstanding exchange codes",
		async () => {
			await withHarness(async ({ asUrl, spotifyManifest }) => {
				const initiate = await initiateGrantRequest(asUrl, spotifyManifest);
				const response = await approveReviewedJson(asUrl, initiate.request_uri);
	...
	await t.test(
		"a revoked grant is not delivered by a stored exchange code",
		async () => {
			await withHarness(async ({ asUrl, spotifyManifest }) => {
				const initiate = await initiateGrantRequest(asUrl, spotifyManifest);
				const approval = await approveReviewedHtml(asUrl, initiate.request_uri);
	...
	await t.test(
		...
		async () => {
			const directory = mkdtempSync(
				join(tmpdir(), "pdpp-consent-handoff-restart-"),
			);
			...
			try {
				...
				assert.ok(
					code,
					`fixture did not mint a code: ${JSON.stringify(minted)}`,
				);
	...
	await t.test(
		...
		async () => {
			await withHarness(async ({ asUrl, spotifyManifest }) => {
				const rollbackInitiated = await initiateGrantRequest(
					asUrl,
					spotifyManifest,
				);
				const rollbackCode = parsePendingConsentRequestUri(
					rollbackInitiated.request_uri,
				);
				...
				await assert.rejects(
					denyGrant(rollbackCode, { faultHook }),
					FORCED_ORDINARY_DENIAL_ROLLBACK_RE,
				);
				assert.ok(
					await getPendingConsent(rollbackCode),
					"rolled-back denial remains pending",
				);
				assert.equal(countConsentEvents(rollbackCode, "consent.denied"), 0);

				const deniedInitiated = await initiateGrantRequest(
					asUrl,
					spotifyManifest,
				);
				const deniedCode = parsePendingConsentRequestUri(
					deniedInitiated.request_uri,
				);
		...
		async () => {
			await withHarness(async ({ asUrl, spotifyManifest }) => {
				...
				const terminal = (await getDb()
					.prepare(
						"SELECT status, token_id FROM pending_consents WHERE device_code = ?",
					)
					.get(deviceCode)) as { status: string; token_id: string | null };
				assert.ok(
					terminal.status === "approved" || terminal.status === "denied",
				);
				assert.equal(terminal.status === "approved", approvedToken !== null);
				assert.equal(
					countConsentEvents(deviceCode, "consent.approved"),
					terminal.status === "approved" ? 1 : 0,
				);
				assert.equal(
					countConsentEvents(deviceCode, "consent.denied"),
					terminal.status === "denied" ? 1 : 0,
				);
