...
/**
 * Import-boundary guard for the `rs.records.list` operation.
 *
 ...
 * `buildLiveRecordsList` -- both pdpp-repo-root frontend paths that do not
 * exist in this repo (`reference-implementation`'s Move B did not bring
 * `apps/site` along; this repo's own `apps/` only has `console`). Removed;
