...
test("published Core Dockerfile stage carries the flag used by the quickstart restart policy", async () => {
  const dockerfile = await read("Dockerfile");
  const compose = await read("deploy/docker/docker-compose.yml");

  // biome-ignore lint/performance/useTopLevelRegex: localized test assertion preserves its explicit contract.
  const coreStageMatch = dockerfile.match(/FROM browsers AS core\n([\s\S]*)/);
  assert.ok(coreStageMatch, "could not isolate the public Core Dockerfile stage");
  // biome-ignore lint/style/useDestructuring: localized test assertion preserves its explicit contract.
  const coreStage = coreStageMatch[1];
  assert.ok(coreStage, "Core stage capture group is empty");

  assert.match(coreStage, new RegExp(`${RESTART_FLAG}=1`));
  // The current Railway console template builds a split console image and
  // does not deploy Core. The supported Core quickstart uses Docker Compose.
  // biome-ignore lint/performance/useTopLevelRegex: localized test assertion preserves its explicit contract.
  const serviceMatch = compose.match(/^  core:\n([\s\S]*?)(?=^  \S|^volumes:)/m);
  assert.ok(serviceMatch, "could not isolate the Core quickstart service");
  assertRealRestartPolicy(serviceMatch[1] ?? "", "published Core quickstart");
});
