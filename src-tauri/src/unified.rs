...
// HOOK(WP-WIRE-BROWSER-LEASE): add the browser-host env pair here when the
// Rust browser-lease module exposes that contract. Keep the hook explicit
// until that module exists; do not guess environment names at this boundary.
...
fn add_browser_host_environment(_env: &mut BTreeMap<OsString, OsString>) {
    // HOOK(WP-WIRE-BROWSER-LEASE): populate the RI host-browser environment
    // pair once the Rust browser-lease module defines its names and values.
}
...
fn ri_process_spec(
    ...
    add_browser_host_environment(&mut env);
    ProcessSpec {
...
fn start_managed_stack(app: &AppHandle, owner_password: &str) -> Result<ManagedStackStart, String> {
    ...
    let ri = Supervisor::new(
        ri_process_spec(&node_binary, &ri_root, &data_dir, owner_password),
        sink.clone(),
    )
    .start()
