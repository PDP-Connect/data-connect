...
The `core` image is built from `deploy/docker/Dockerfile` target `core`.

**Architecture note (differs from the pdpp original this was ported from):**
in pdpp, this image's supervisor ran the operator console on Railway's
injected `$PORT` plus the reference AS/RS on loopback (`127.0.0.1:7662` /
`127.0.0.1:7663`) in one container. data-connect's `core` stage does not
bundle the console — it runs only the reference AS/RS, directly on `$PORT`
(the Dockerfile maps Railway's injected `PORT` onto `AS_PORT`; see that
Dockerfile's `reference`/`core` stage `CMD`). The rest of this document's
"console on $PORT, AS/RS on loopback" framing describes the pdpp original,
not this repo's current `core` image; treat it as the aspirational shape
`deploy/railway/core-supervisor.ts` (ported, but not wired into
`deploy/docker/Dockerfile`) would restore if adopted here.
