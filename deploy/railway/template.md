...
|---|---|---:|---|
| `core` | `ghcr.io/pdp-connect/data-connect/core:latest` (or a pinned `<version-tag>`) | enabled | Railway default with browser connectors; externally probe `/.well-known/oauth-authorization-server` |
| `Postgres` | Railway plugin | disabled | n/a |

The `core` image is built from the repository-root `Dockerfile` target `core`.
Its supervisor runs the console on Railway's injected `$PORT` and the
reference AS/RS on loopback (`127.0.0.1:7662` / `127.0.0.1:7663`) in one
container. The console proxies owner and protocol requests to the reference
server.

This is the selected SLVP for the public button because live testing showed
that a separate private `reference` image service requires an explicit
service `PORT` to boot reliably, and Railway turns that `PORT` into an extra
required deploy prompt. The one-service image preserves one public origin
without asking the deploying operator for topology constants.

Choose the image reference by intent:

- `ghcr.io/pdp-connect/data-connect/core:latest` — the moving public image path. It
...
- `ghcr.io/pdp-connect/data-connect/core:<version-tag>` — a concrete immutable tag
  (for example `core:1.5.1`). Pin one, never a moving tag, when a given template
  revision must be byte-for-byte reproducible. Manual diagnostic images use
...
The probe exits `0` only when `ghcr.io/pdp-connect/data-connect/core:<version-tag>`
...
`/setup` with the one-time token in the Core deploy logs. Operators who automate
deployment can set it as a Railway secret; a configured value skips the wizard.
...
2. Add the `core` service from Docker Image
   `ghcr.io/pdp-connect/data-connect/core:latest`, or a pinned
   `ghcr.io/pdp-connect/data-connect/core:<version-tag>` if this revision must be
   reproducible.
3. Add a Railway Postgres plugin.
4. Configure variables exactly as listed above.
5. Generate a public domain for `core`.
6. Open `/setup`, use the one-time token from Core deploy logs, and choose an
   owner password.
...
9. Generate a template from the validated project.
10. Inspect the generated template config. It should not require a
   user-provided owner password; `core.PDPP_CREDENTIAL_ENCRYPTION_KEY` must be
   generated by the template with `${{ secret(64) }}`. It must not ask for
   `PORT`, `AS_PORT`, `RS_PORT`, `PDPP_AS_URL`, or `PDPP_RS_URL`.
11. Publish the template from the workspace template list.
12. Deploy a fresh scratch project from the published template and rerun the
    live smoke plus restart smoke.
13. Replace any placeholder template code in the chosen user-facing button
...
- [x] `npm run railway:ghcr-public -- --tag <version-tag>` exits `0` for the tag the
...
- `core` image: `ghcr.io/pdp-connect/data-connect/railway-core:sha-6581820`.
...
npm run railway:ghcr-public:test
