...
this runbook describes pdpp's combined Core image, where a supervisor process
ran both the operator console (on Railway's `$PORT`) and the reference AS/RS
(on loopback) in one container. data-connect's `deploy/docker/Dockerfile`
`core` stage does not bundle the console today — it runs only the reference
AS/RS, directly on `$PORT`. Everywhere below that says "console", read that as
"the reference AS/RS" for this repo; the console (`apps/console`) deploys as
its own separate Railway service today (see `railway.console.json`,
`console.env.example`), not folded into `core`. See
`deploy/docker/README.md`'s own architecture note for the fuller explanation,
and `deploy/railway/core-supervisor.ts` for the ported (but currently
unwired) supervisor that would restore the combined-image behavior.
