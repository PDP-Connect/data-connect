...
const EXPECTED_RELAY_ORIGIN = new URL(import.meta.env.VITE_SESSION_RELAY_URL)
  .origin
...
describe("claimSession", () => {
  ...
  it("throws generic error when HTTP error has no structured body", async () => {
    fetchSpy.mockResolvedValueOnce(jsonResponse({ unexpected: "format" }, 500))
...
describe("denySession", () => {
  ...
  it("throws on INVALID_SESSION_STATE", async () => {
    ...
    await expect(denySession("sess-1", { secret: "s" })).rejects.toMatchObject({
      errorCode: "INVALID_SESSION_STATE",
    })
  })
})
