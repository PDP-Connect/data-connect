 ...
 * 2. Signs an EIP-712 ServerRegistration message via the configured account service
...
export async function registerServer(
  ...
  const tunnelDomain = configuredTunnelAddress(
    import.meta.env.VITE_TUNNEL_SERVER_ADDR
  ).replace(/^frpc\./, "")
  ...
  // Get the owner's signature via the configured account service.
  const signature = await signTypedData(masterKeySignature, typedData)

  // POST to gateway
  const res = await tauriFetch(`${gatewayUrl()}/v1/servers`, {
    method: "POST",
    headers: {
  ...
  // A 409 means already registered, so treat it as success.
  if (res.status === 409) {
    const body = await res.json().catch(() => ({}))
    return {
      serverId: ((body as Record<string, unknown>).serverId as string) ?? null,
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Gateway server registration failed (${res.status}): ${text}`
    )
  }
