...
const { bareImportSpecifiers, bareSpecifierPackageName } =
  await import("../scripts/validate-package.ts")
...
`
  assert.deepEqual(
    [...bareImportSpecifiers(source)],
    ["@pdpp/reference-contract/common"]
  )
  assert.equal(
    bareSpecifierPackageName("@pdpp/reference-contract/common"),
...
test("bareSpecifierPackageName resolves scoped and unscoped package names from a subpath specifier", () => {
  ...
  assert.equal(
    bareSpecifierPackageName("@pdpp/reference-contract"),
    "@pdpp/reference-contract"
  )
  assert.equal(
    bareSpecifierPackageName("@pdpp/reference-contract/common"),
...
test("bareImportSpecifiers recognizes the bare side-effect import form", () => {
  ...
  assert.deepEqual(
    [...bareImportSpecifiers(source)],
    ["@pdpp/reference-contract/register"]
  )
})
